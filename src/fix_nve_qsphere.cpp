...
FixNVEQSphere::FixNVEQSphere(LAMMPS *lmp, int narg, char **arg) :
  FixNVE(lmp, narg, arg)
{
...
void FixNVEQSphere::initial_integrate(int vflag)
{
  double dtfm,dtirotate,msq,scale;
  double g[3];
  double inertia[3],angmom[3]; // Added angmom to this fix

  double **x = atom->x;
  double **v = atom->v;
  double **f = atom->f;
  double **omega = atom->omega;
  // Added quat to this fix
  ...
  for (int i = 0; i < nlocal; i++) {
    if (mask[i] & groupbit) {
      dtfm = dtf / rmass[i];
      inertia_tmp = (radius[i]*radius[i]*rmass[i]); // FIX FOR 3D!
      v[i][0] += dtfm * f[i][0];
      v[i][1] += dtfm * f[i][1];
      v[i][2] += dtfm * f[i][2];
      x[i][0] += dtv * v[i][0];
      ...
      angmom[0] = omega[i][0] * inertia_tmp * INERTIA;
      angmom[1] = omega[i][1] * inertia_tmp * INERTIA;
      angmom[2] = omega[i][2] * inertia_tmp * INERTIA;

      // Moment of inertia of a sphere

      inertia[0] = INERTIA * inertia_tmp; 
      inertia[1] = INERTIA * inertia_tmp; 
      inertia[2] = INERTIA * inertia_tmp; 

      //MathExtra::mq_to_omega(angmom[i],quat,inertia,omega);
      MathExtra::richardson(quat[i],angmom,omega[i],inertia,dtq);
    }
  }
...
void FixNVEQSphere::final_integrate()
{
