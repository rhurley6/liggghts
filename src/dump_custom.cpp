...
enum{ID,MOL,TYPE,ELEMENT,MASS,
     ...
     SPHQUAT0,SPHQUAT1,SPHQUAT2,SPHQUAT3,
...
int DumpCustom::parse_fields(int narg, char **arg)
{
  ...
  for (int iarg = 5; iarg < narg; iarg++) {
    ...
    if (strcmp(arg[iarg],"id") == 0) {
      ...
      if (!atom->quat_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_sphquat0;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"sphquat1") == 0) {
      if (!atom->quat_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_sphquat1;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"sphquat2") == 0) {
      if (!atom->quat_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_sphquat2;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"sphquat3") == 0) {
      if (!atom->quat_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustom::pack_sphquat3;
      vtype[i] = DOUBLE;
...
void DumpCustom::pack_sphquat0(int n)
{
...
void DumpCustom::pack_sphquat1(int n)
{
  double **quat = atom->quat;
...
void DumpCustom::pack_sphquat2(int n)
{
  double **quat = atom->quat;
...
void DumpCustom::pack_sphquat3(int n)
{
  double **quat = atom->quat;
