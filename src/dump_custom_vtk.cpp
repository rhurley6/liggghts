  ...
  if (nthresh) {
    ...
    for (int ithresh = 0; ithresh < nthresh; ithresh++) {
      ...
      if (thresh_array[ithresh] == ID) {
        ...
        ptr = &atom->omega[0][2];
        nstride = 3;
      } else if (thresh_array[ithresh] == SPHQUAT0) {
        if (!atom->quat_flag)
          error->all(FLERR,
                     "Threshhold for an atom property that isn't allocated");
        ptr = &atom->quat[0][0];
        nstride = 4;
      } else if (thresh_array[ithresh] == SPHQUAT1) {
        if (!atom->quat_flag)
          error->all(FLERR,
                     "Threshhold for an atom property that isn't allocated");
        ptr = &atom->quat[0][1];
        nstride = 4;
      } else if (thresh_array[ithresh] == SPHQUAT2) {
        if (!atom->quat_flag)
          error->all(FLERR,
                     "Threshhold for an atom property that isn't allocated");
        ptr = &atom->quat[0][2];
        nstride = 4;
      } else if (thresh_array[ithresh] == SPHQUAT3) {
        if (!atom->quat_flag)
          error->all(FLERR,
                     "Threshhold for an atom property that isn't allocated");
        ptr = &atom->quat[0][3];
...
int DumpCustomVTK::parse_fields(int narg, char **arg)
{
  ...
  for (int iarg = 5; iarg < narg; iarg++) {
    ...
    } else if (strcmp(arg[iarg],"sphquat0") == 0) {
      if (!atom->quat_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[SPHQUAT0] = &DumpCustomVTK::pack_sphquat0;
      vtype[SPHQUAT0] = DOUBLE;
      name[SPHQUAT0] = arg[iarg];
    } else if (strcmp(arg[iarg],"sphquat1") == 0) {
      if (!atom->quat_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[SPHQUAT1] = &DumpCustomVTK::pack_sphquat1;
      vtype[SPHQUAT1] = DOUBLE;
      name[SPHQUAT1] = arg[iarg];
    } else if (strcmp(arg[iarg],"sphquat2") == 0) {
      if (!atom->quat_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[SPHQUAT2] = &DumpCustomVTK::pack_sphquat2;
      vtype[SPHQUAT2] = DOUBLE;
      name[SPHQUAT2] = arg[iarg];
    } else if (strcmp(arg[iarg],"sphquat3") == 0) {
      if (!atom->quat_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[SPHQUAT3] = &DumpCustomVTK::pack_sphquat3;
      vtype[SPHQUAT3] = DOUBLE;
      name[SPHQUAT3] = arg[iarg];
    ...
    // if no trailing [], then arg is set to 0, else arg is int between []
      ...
      if (!atom->superquadric_flag)
        error->all(FLERR,"Dumping an atom quantity that isn't allocated");
      pack_choice[ROUNDNESS1] = &DumpCustomVTK::pack_roundness1;
      vtype[ROUNDNESS1] = DOUBLE;
      name[ROUNDNESS1] = arg[iarg];
    } else if (strcmp(arg[iarg],"roundness2") == 0) {
      if (!atom->superquadric_flag)
        error->all(FLERR,"Dumping an atom quantity that isn't allocated");
      pack_choice[ROUNDNESS2] = &DumpCustomVTK::pack_roundness2;
...
void DumpCustomVTK::identify_vectors()
{
  ...
  int vector3_starts[] = {XS, XU, XSU, IX, VX, FX, MUX, OMEGAX, SPHQUAT0, ANGMOMX, TQX};
...
void DumpCustomVTK::identify_tensor()
{
  if(pack_choice.count(QUAT1) > 0 &&
     ...
     pack_choice.count(QUAT4) > 0) {
       ...
       pack_choice[TENSOR] = &DumpCustomVTK::pack_tensor;
...
int DumpCustomVTK::modify_param(int narg, char **arg)
{
  ...
  if (strcmp(arg[0],"thresh") == 0) {
    ...
    else if (strcmp(arg[1],"sphquat0") == 0) thresh_array[nthresh] = SPHQUAT0;
    else if (strcmp(arg[1],"sphquat1") == 0) thresh_array[nthresh] = SPHQUAT1;
    else if (strcmp(arg[1],"sphquat2") == 0) thresh_array[nthresh] = SPHQUAT2;
    else if (strcmp(arg[1],"sphquat3") == 0) thresh_array[nthresh] = SPHQUAT3;
...
void DumpCustomVTK::pack_density(int n) 
{
...
void DumpCustomVTK::pack_p(int n) 
{
  double *p = atom->p;
...
void DumpCustomVTK::pack_rho(int n) 
{
  double *rho = atom->rho;
...
void DumpCustomVTK::pack_sphquat0(int n)
{
  double **quat = atom->quat;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = quat[clist[i]][0];
    n += size_one;
  }
}
...
void DumpCustomVTK::pack_sphquat1(int n)
{
  double **quat = atom->quat;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = quat[clist[i]][1];
    n += size_one;
  }
}
...
void DumpCustomVTK::pack_sphquat2(int n)
{
  double **quat = atom->quat;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = quat[clist[i]][2];
    n += size_one;
  }
}
...
void DumpCustomVTK::pack_sphquat3(int n)
{
  double **quat = atom->quat;

  for (int i = 0; i < nchoose; i++) {
    buf[n] = quat[clist[i]][3];
    n += size_one;
  }
}
...
void DumpCustomVTK::pack_roundness1(int n)
{
...
void DumpCustomVTK::pack_roundness2(int n)
{
  double **roundness = atom->roundness;
...
void DumpCustomVTK::pack_tensor(int n)
{
