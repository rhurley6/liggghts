...
#include "stdlib.h"
#include "string.h"
#include "fix_template_qsphere.h"
#include "atom.h"
#include "atom_vec.h"
#include "modify.h"
#include "memory.h"
...
#define LMP_DEBUGMODE_QSPHERE false

/* ---------------------------------------------------------------------- */

FixTemplateQSphere::FixTemplateQSphere(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg)
{
  ...
  while (iarg < narg && hasargs)
  {
    ...
    else if (strcmp(arg[iarg],"volume_limit") == 0)
    {
      ...
      hasargs = true;
      if(strcmp(this->style,"particletemplate/qsphere"))
    ...
    else if(strcmp(style,"particletemplate/qsphere") == 0)
  ...
  if(strcmp(this->style,"particletemplate/qsphere"))return;
...
FixTemplateQSphere::~FixTemplateQSphere()
{
    ...
    if(strcmp(style,"particletemplate/qsphere") == 0)
    {
        delete pti;
...
int FixTemplateQSphere::setmask()
{
  int mask = 0;
  return mask;
}
...
Region* FixTemplateQSphere::region()
{
...
void FixTemplateQSphere::randomize_single()
{
...
void FixTemplateQSphere::init_ptilist(int n_random_max)
{
...
void FixTemplateQSphere::delete_ptilist()
{
...
void FixTemplateQSphere::randomize_ptilist(int n_random,int distribution_groupbit,int distorder)
{
    for(int i = 0; i < n_random; i++)
    {
        ...
        vectorZeroize3D(pti_list[i]->quat_ins);
...
double FixTemplateQSphere::min_rad()
{
...
double FixTemplateQSphere::max_rad()
{
...
double FixTemplateQSphere::max_r_bound()
{
    return pdf_max(pdf_radius);
}
...
double FixTemplateQSphere::volexpect()
{
...
double FixTemplateQSphere::massexpect()
{
    return mass_expect;
}
...
int FixTemplateQSphere::number_spheres()
{
    return 1;
}
...
int FixTemplateQSphere::maxtype()
{
    return atom_type;
}
...
int FixTemplateQSphere::mintype()
{
    return atom_type;
}
...
void FixTemplateQSphere::write_restart(FILE *fp)
{
  int n = 0;
...
void FixTemplateQSphere::restart(char *buf)
{
